...
export async function addProduct(productData: Omit<Product, 'id' | 'rating' | 'reviews' | 'features'>): Promise<Product | null> {
  try {
    const { data, error } = await supabase
      .from('products')
      .insert({
...
export async function updateProduct(productId: number, productData: Partial<Omit<Product, 'id' | 'rating' | 'reviews' | 'features'>>): Promise<Product | null> {
  try {
    const { data, error } = await supabase
      .from('products')
      .update({
